...
def say_welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        '<b>Hello! This is a telegram bot template written by <a href="https://github.com/vicktorM4A4">zxvizi</a></b>',
        parse_mode='html'
    )
...
async def bot_message(message):
    if message.text == 'Личный кабинет':
        await bot.send_message(message.from_user.id, 'https://olimpiyasport.ru/#fitnesskit_personal',reply_markup=app.mainMenu)
        ...
        await bot.send_message(message.from_user.id, 'Наши контакты:',reply_markup=app.otherMenu)

    elif message.text == 'Главное меню':
        await bot.send_message(message.from_user.id, ' Возврат ',reply_markup=app.mainMenu)
